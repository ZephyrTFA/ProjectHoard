...
{
    private static readonly Dictionary<Type, ModuleCommandMap> CommandMaps = new();
    ...
    public static void DropModuleCommands(Type module)
    {
        CommandMaps.Remove(module);
    }
    ...
    public static async Task ClearCommandsForShutdown()
    {
        ...
        foreach (var guild in
                 HoardMain.DiscordClient.Guilds.Select(guild => HoardMain.DiscordClient.GetGuild(guild.Id)))
    ...
    public static async Task DiscordClientOnSlashCommandExecuted(SocketSlashCommand slashCommand)
    {
        ...
        var moduleType =
            CommandMaps.Keys.FirstOrDefault(type => type.GetNormalizedRepresentation().Equals(moduleData.Name));
        ...
        catch (TimeoutException)
        {
            HoardMain.Logger.LogWarning(
                "A slash command has taken longer than five seconds to return control to the gateway!");
        }
        catch (Exception exception)
        {
            HoardMain.Logger.LogWarning(exception, "A slash command ('{}') experienced an exception during runtime",
                commandData.Name);

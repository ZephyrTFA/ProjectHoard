...
{
    internal static readonly Dictionary<string, Type> TypeMap = new();
    internal static readonly Dictionary<Type, ModuleBase> InstanceMap = new();
    private static readonly Dictionary<Type, uint> LoadCount = new();
    private static readonly Dictionary<ulong, ICollection<Type>> LoadedMap = new();
    ...
    public static ModuleLoadResult TryLoadModule(ulong guild, string moduleName, out Exception? exception,
    ...
    {
        await DoForAll(message.Channel.GetGuildId(),
            async module => await module.DiscordClientOnMessageReceived(message));
    }

    public static async Task DiscordClientOnMessageDeleted(Cacheable<IMessage, ulong> message,
        Cacheable<IMessageChannel, ulong> channel)
    {
        ...
        await DoForAll(channelActual.GetGuildId(),
            async module => await module.DiscordClientOnMessageDeleted(socketMessage, channelActual));
    }

    public static async Task DiscordClientOnMessagesBulkDeleted(
        IReadOnlyCollection<Cacheable<IMessage, ulong>> messages, Cacheable<IMessageChannel, ulong> channel)
    {
        var messagesActual = messages.Where(message => message.HasValue).Select(message => (SocketMessage)message.Value)
            .ToList().AsReadOnly();
        var channelActual = (ISocketMessageChannel)await channel.GetOrDownloadAsync();
        await DoForAll(channelActual.GetGuildId(),
            async module => await module.DiscordClientOnMessagesBulkDeleted(messagesActual, channelActual));
    }

    public static async Task DiscordClientOnMessageUpdated(Cacheable<IMessage, ulong> oldMessage,
        SocketMessage newMessage, ISocketMessageChannel channel)
    {
        if (!oldMessage.HasValue || oldMessage.Value is not SocketMessage socketMessage)
            return;
        await DoForAll(channel.GetGuildId(),
            async module => await module.DiscordClientOnMessageUpdated(socketMessage, newMessage, channel));
    }

    public static async Task DiscordClientOnUserJoined(SocketGuildUser user)
    {
        await DoForAll(user.Guild.Id, async module => await module.DiscordClientOnUserJoined(user));
    }

    public static async Task DiscordClientOnUserLeft(SocketGuild guild, SocketUser user)
    {
        await DoForAll(guild.Id, async module => await module.DiscordClientOnUserLeft(guild, user));
    }

    public static async Task DiscordClientOnUserUpdated(SocketUser oldUser, SocketUser newUser)
    {
        await DoForAll(0, async module => await module.DiscordClientOnUserUpdated(oldUser, newUser));
    }

    public static async Task DiscordClientOnGuildMemberUpdated(Cacheable<SocketGuildUser, ulong> oldUser,
        SocketGuildUser newUser)
    {
        if (!oldUser.HasValue)
            return;
        await DoForAll(newUser.Guild.Id,
            async module => await module.DiscordClientOnGuildMemberUpdated(oldUser.Value, newUser));
    }
    ...
    {
        await DoForAll(0, async module => await module.DiscordClientOnJoinedGuild(guild));
    }

    public static async Task DiscordClientOnLeftGuild(SocketGuild guild)
    {
        await DoForAll(0, async module => await module.DiscordClientOnLeftGuild(guild));
    }

    public static async Task DiscordClientOnUserBanned(SocketUser user, SocketGuild guild)
    {
        await DoForAll(guild.Id, async module => await module.DiscordClientOnUserBanned(user, guild));
    }

    public static async Task DiscordClientOnUserUnbanned(SocketUser user, SocketGuild guild)
    {
        await DoForAll(guild.Id, async module => await module.DiscordClientOnUserUnbanned(user, guild));
    }

    public static async Task DiscordClientOnInviteCreated(SocketInvite invite)
    {
        await DoForAll(invite.Guild.Id, async module => await module.DiscordClientOnInviteCreated(invite));
    }

    public static async Task DiscordClientOnInviteDeleted(SocketGuildChannel oldInviteChannel, string oldInviteUrl)
    {
        await DoForAll(oldInviteChannel.GetGuildId(),
            async module => await module.DiscordClientOnInviteDeleted(oldInviteChannel, oldInviteUrl));
    }

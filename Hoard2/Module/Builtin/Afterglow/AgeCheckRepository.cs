...
public class AgeCheckRepository : ModuleBase
{
    ...
    private List<AgeCheckInformation> GetVerifications(ulong guild)
    {
        return GuildConfig(guild).Get("age-checks", new List<AgeCheckInformation>())!;
    }

    private void SetVerifications(ulong guild, List<AgeCheckInformation> checks)
    {
        GuildConfig(guild).Set("age-checks", checks);
    }
    ...
    public async Task SetVerificationRole(SocketSlashCommand command, IRole role)
    {
        ...
        await command.RespondAsync($"Updated the verification role to <@&{role.Id}>.",
    ...
    public async Task SetAgeCheckedRole(SocketSlashCommand command, IRole role)
    {
        ...
        await command.RespondAsync($"Updated the age check role to <@&{role.Id}>.",
            allowedMentions: AllowedMentions.None);
    }
    ...
    public async Task VerifyMember(SocketSlashCommand command, IUser user, bool ageChecked = false)
    {
        ...
        {
            await command.RespondAsync($"{user.Mention} is already verified.", ephemeral: true,
                allowedMentions: AllowedMentions.None);
        ...
        await command.SendOrModifyOriginalResponse($"Verified{(ageChecked ? " and Age Checked" : "")} {user.Mention}.",
    ...
    public async Task GetVerifiedStatus(SocketSlashCommand command, IUser user)
    {
        ...
        await command.SendOrModifyOriginalResponse(
            $"{user.Mention} was verified{(verifyEntry.AgeChecked ? " and age checked" : "")} by <@{verifyEntry.Verifier}>(<t:{verifyEntry.When}:R>).",
            AllowedMentions.None);
    }

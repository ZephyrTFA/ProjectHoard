...
public class MemberFlagger : ModuleBase
{
    ...
    private void SetLogChannel(ulong guild, ulong channelId)
    {
        GuildConfig(guild).Set("log-channel", channelId);
    }

    private List<ulong> GetIgnoreList(ulong guild)
    {
        return GuildConfig(guild).Get("ignore-list", new List<ulong>())!;
    }

    private void SetIgnoreList(ulong guild, List<ulong> list)
    {
        GuildConfig(guild).Set("ignore-list", list);
    }
    ...
    private void SetFlagRole(ulong guild, ulong roleId)
    {
        GuildConfig(guild).Set("flag-role", roleId);
    }
    ...
    {
        await ProcessGuildUser(socketGuildUser);
    }

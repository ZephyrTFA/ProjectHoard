...
public class MessageLog : ModuleBase
{
    ...
    public override async Task DiscordClientOnMessagesBulkDeleted(ReadOnlyCollection<SocketMessage> messages,
    ...
    public override async Task DiscordClientOnMessageUpdated(SocketMessage oldMessage, SocketMessage newMessage,
        ISocketMessageChannel channel)
    {
    ...
    private bool TryGetChannel(ulong guild, [NotNullWhen(true)] out ISocketMessageChannel? channel)
    {
        ...
        if (!GuildConfig(guild).TryGet("log-channel", out ulong channelId))
            return false;
        channel =
            HoardMain.DiscordClient.GetChannelAsync(channelId).Preserve().GetAwaiter().GetResult() as

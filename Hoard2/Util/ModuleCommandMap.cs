...
public class ModuleCommandInformation
{
    ...
    private ModuleCommandInformation(string name, string desc, ImmutableList<ParameterInformation> parameters,
        MethodInfo caller, GuildPermission? permissions, bool guildOnly, bool dmOnly)
    {
        Name = name;
        Desc = desc;
    ...
    public static ModuleCommandInformation GenerateCommandInformation(MethodInfo command)
    {
        ...
        var permissions = command.GetCustomAttribute<ModuleBase.ModuleCommandAttribute>()!
            .CommandPermissionRequirements;
...
public class ModuleCommandMap
{
    ...
    {
        var commandMethods = moduleType.GetMethods()
            .Where(method => method.GetCustomAttribute<ModuleBase.ModuleCommandAttribute>() is not null).ToList();
